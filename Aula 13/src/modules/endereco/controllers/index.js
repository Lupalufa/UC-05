...
class EnderecoController{
    ...
    static async listarEnderecoAlunos(req,res){
        try {
            ...
            const endereco = await EnderecoModel.listarEnderecoAlunos(matricula)
            if (endereco.length === 0) {
                return res.status(400).json({msg: "Não há nenhum endereço com esse aluno"})
            }
            res.status(200).json(endereco)
        } catch (error) {
            res.status(500).json({ msg: "Erro ao listar por aluno", erro: error.message })
    ...
    //         const endereco = await EnderecoModel.listarEnderecoAlunos(matricula)
    //         if (endereco.length === 0) {
    //             return res.status(400).json({msg: "Não há nenhum endereço com essa Matricula"})
    //         }
    //         res.status(200).json(endereco)
    //     } catch (error) {
    //         res.status(500).json({ msg: "Erro ao listar o aluno", erro: error.message })
    ...
    static async listarEnderecoCEP(req, res){
        try {
            ...
            if (endereco.length === 0) {
                return res.status(400).json({msg: "Não há nenhum endereço com esse CEP"})
            }
            res.status(200).json(endereco)
        } catch (error) {
    ...
    static async listarEnderecoCidade(req, res){
        try {
            ...
            if (endereco.length === 0) {
                return res.status(400).json({msg: "Não há nenhum endereço com essa Cidade"})
            }
            res.status(200).json(endereco)
        } catch (error) {
            res.status(500).json({ msg: "Erro ao listar por cidade!", erro: error.message })
    ...
    static async adicionarEnderecoAluno(req, res){
        try {
            ...
            res.status(500).json({ msg: "Erro ao criar por aluno!", erro: error.message })
